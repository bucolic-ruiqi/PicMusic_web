...
export default function ProfileClient({ initial }: { initial: { userId: number; username: string; email: string } }) {
  const { userId } = initial;
  const [username, setUsername] = useState(initial.username || "");
  const [account, setAccount] = useState(initial.email || "");
  ...
  // 初始化从 localStorage 读取可选项（头像、MBTI、音乐风格），用户名/邮箱以服务端为准
  useEffect(() => {
    try {
  ...
  async function saveProfile(next: { username?: string; email?: string }) {
    setSaving(true);
    try {
      ...
      if (typeof data.email === "string" || data.email === null) setAccount(data.email ?? "");
    } catch (e) {
        ...
        <div className="flex items-start gap-6">
          ...
          <div className="mt-1 flex min-w-[4rem] flex-col items-start gap-2">
            <div className="relative h-16 w-16 overflow-hidden rounded-full ring-2 ring-brand-300 dark:ring-brand-400/30">
              {avatarSrc ? (
                <Image src={avatarSrc} alt={username} fill sizes="64px" className="object-cover" />
              ) : (
                <div className="flex h-full w-full items-center justify-center bg-gradient-to-br from-brand-600 to-brand-400 text-zinc-700">
                  <span className="text-lg font-semibold">{username?.[0] || "U"}</span>
                </div>
              )}
            </div>
            <button
          ...
          <div className="min-w-0 flex-1">
            ...
            {!editingUsername ? (
              <div className="flex items-center gap-2">
                <h1 className="text-xl font-semibold tracking-tight text-zinc-900 dark:text-zinc-50">{username}</h1>
                <button
              ...
              <div className="flex items-center gap-2">
                ...
                />
                <button
                  className="rounded-full bg-brand-700 px-2 py-0.5 text-xs font-medium text-white hover:bg-brand-800 disabled:opacity-60"
                  disabled={saving}
                  onClick={() => { setEditingUsername(false); saveProfile({ username: draftUsername }); }}
                >
                  保存
                </button>
                <button
            ...
            {!editingAccount ? (
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  className="rounded-full bg-brand-700 px-2 py-0.5 text-xs font-medium text-white hover:bg-brand-800 disabled:opacity-60"
                  disabled={saving}
                  onClick={() => { setEditingAccount(false); saveProfile({ email: draftAccount }); }}
                >
                  保存
                </button>
                <button
